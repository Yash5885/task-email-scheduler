...
$('#saveSchedule').addEventListener('click', async () => {
  try {
    ...
    setSyncState(friendlyError(error), 'bad');
  }
});
...
function friendlyError(error) {
  ...
  if (message.includes('SMTP 535') || message.includes('BadCredentials')) {
    return 'Company Gmail rejected the SMTP login. Create a fresh app password for yashamantrial@gmail.com, update COMPANY_SMTP_PASSWORD in Render, and redeploy.';
  }

  if (message.includes('Company sender is not configured')) {
    return 'Company sender is missing on the server. Set COMPANY_SMTP_PASSWORD in Render, then redeploy.';
  }

  if (message.includes('SMTP') || message.includes('smtp.gmail.com')) {
